...
class FieldTestPayment extends StatelessWidget {
  const FieldTestPayment({super.key});

  @override
  Widget build(BuildContext context) {
    return Scaffold(
        appBar: appBar(context: context, title: "Field Test Payment"),
        body: userCntr.allUsers != null
            ? SizedBox(
                height: 80.h,
                child: Obx(() {
                  List<UserModel> filteredList = userCntr.allUsers!
                      .where((p0) => p0.userType == AllStrings.fieldPaymentType)
                      .toList();
